...
bool simulate(MouseEvent event) {
  ...
  if (int.parse(numInput.value, onError: (e) => null) == null) {
    return false;
  }
  ...
  bool limitScope =
      (querySelector("#limitScope") as CheckboxInputElement).checked;
  if (limitScope) {
    updateScope();
  }
...
updateScope() {
  if ((querySelector("#radioScope0") as RadioButtonInputElement).checked) {
    scope = int.parse(
        (querySelector("#radioScope0") as RadioButtonInputElement).value);
  } else if ((querySelector("#radioScope1") as RadioButtonInputElement)
      .checked) {
    scope = int.parse(
        (querySelector("#radioScope1") as RadioButtonInputElement).value);
  } else {
    scope = int.parse(
        (querySelector("#radioScope2") as RadioButtonInputElement).value);
  }
}
